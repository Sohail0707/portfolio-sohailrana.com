          ...
          {services.map((service, i) => {
            ...
            return (
              <Reveal key={service.title} delay={i * 0.1}>
                <article
                  className={`group h-full rounded-2xl border border-line bg-panel p-6 transition-colors sm:p-7 md:p-8 ${accent.border}`}
                >
                  <div className="flex items-start justify-between">
                    <accent.Icon className={`icon-draw h-9 w-9 sm:h-10 sm:w-10 ${accent.text}`} />
                    <span className={`font-mono text-sm ${accent.text}`}>
                      {String(i + 1).padStart(2, "0")}
                    </span>
                  </div>
                  <h3 className="mt-5 font-display text-xl font-bold sm:text-2xl">{service.title}</h3>
